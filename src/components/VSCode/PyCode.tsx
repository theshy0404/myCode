...
const tipWords = ['return', 'if', 'else', 'try', 'catch', 'True', 'False',];
...
class VSCode extends React.Component<any, TState>{
    ...
    run(): void {
        ...
        const func = 'search';
        const input = 'nums = [-1,0,3,5,9,12], target = 9';
        for (let line of this.state.codeLines) {
            let lineCode = line.code.replace('\t', '');
            if (lineCode.indexOf("def") !== 0) lineCode = '\t\t\t\t' + lineCode;
            code += lineCode;
        }
        axios.post('http://127.0.0.1:5000/problem/runPy', {
            params: {
                code, language, input, func
            }
