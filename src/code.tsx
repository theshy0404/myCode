...
class App extends React.Component<any, TState>{
  constructor(props: any) {
    ...
    this.state = {
      codeLines: [
        { code: 'function sum(a,b) {', isEditable: false },
  ...
  handleLineEnter(event: any, lineIndex: number): void {
    ...
    if (event.keyCode >= 65 && event.keyCode <= 90) {
      return this.setState({ word: word += event.key, words: [...keyWords,...globalWords].filter(item => item.indexOf(word) !== -1), selectTip: 0 });
    }
    if (keyCode === 13) {
  ...
  handleChangeLineValue(event: any, lineIndex: number): void {
    ...
    if (changeLine) {
      ...
      const positionLeft = changeLine.offsetLeft + 40 + event.target.value.length * 7.04;
      return this.setState({
  ...
  doTest(): void {
    const { codeLines, input } = this.state;
    let doScript = '(';
    codeLines.forEach(item => doScript += item.code);
    doScript += `)(${input.toString()})`;
    let output;
    try {
      output = new_eval(doScript);
    }
    catch (e: any) {
      this.setState({ isHasError: true }, () => message.error(e.toString()));
      return;
    }
    this.setState({ output, isHasError: false })
  }

  render() {
    const lines = this.state.codeLines;
    const { input, output, isHasError, positionTop, positionLeft, word, words, selectTip } = this.state;
    ...
    return (
      <div className={styles.wrap}>
        <div className={styles.codeTips} style={{ position: 'absolute', top: positionTop, left: positionLeft, display: word === '' ? 'none' : 'block' }}>
          {words.map((item, index) =>
            <div data-select={selectTip === index} key={index} className={styles.tipWord}>{item}</div>
          )}
        </div>
        <div className={styles.aside}>
          <Button onClick={() => this.doTest()}>测试</Button>
          <List
            header={<div>输入</div>}
            bordered
            dataSource={input}
          ...
          <List
            header={<div>输出</div>}
            bordered
            dataSource={[isHasError ? '出错了哦' : output]}
            renderItem={item => (
              <List.Item>
                {item}
              </List.Item>
        ...
        <div className={styles.codeWrap}>
          <div className={styles.codeIndex}>
            {lines.map((item, index) => <div key={index} className={styles.index}>{index}</div>)}
          </div>
